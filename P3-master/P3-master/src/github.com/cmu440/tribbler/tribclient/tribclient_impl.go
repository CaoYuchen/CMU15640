...
func (tc *tribClient) ModifyTribble(userID, postKey, newContents string) (tribrpc.Status, error) {
	args := &tribrpc.ModifyTribbleArgs{UserID: userID, PostKey: postKey, Contents: newContents}
	var reply tribrpc.ModifyTribbleReply
	if err := tc.client.Call("TribServer.ModifyTribble", args, &reply); err != nil {
		return 0, err
	}
	return reply.Status, nil
}
