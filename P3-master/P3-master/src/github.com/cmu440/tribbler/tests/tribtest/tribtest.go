...
func initTribServer(masterServerHostPort, proxyServerHostPort, tribServerHostPort string) error {
	proxyCounter, err := proxycounter.NewProxyCounter(masterServerHostPort, proxyServerHostPort)
	if err != nil {
	...
	l, err := net.Listen("tcp", proxyServerHostPort)
	if err != nil {
	...
	tribServer, err := tribserver.NewTribServer(proxyServerHostPort, tribServerHostPort, libstore.Normal, false)
	if err != nil {
...
func checkTribbles(tribbles, expectedTribbles []tribrpc.Tribble) bool {
	...
	for i := len(tribbles) - 1; i >= 0; i-- {
		if tribbles[i].UserID != expectedTribbles[i].UserID {
			LOGE.Printf("FAIL (UserID): incorrect tribbles %v, expected tribbles %v\n", tribbles, expectedTribbles)
			failCount++
			return true
		}
		if tribbles[i].Contents != expectedTribbles[i].Contents {
			LOGE.Printf("FAIL (Contents): incorrect tribbles %v, expected tribbles %v\n", tribbles, expectedTribbles)
			failCount++
			return true
		}
...
func modifyTribble(user, postKey, newContents string) (error, tribrpc.Status) {
	args := &tribrpc.ModifyTribbleArgs{UserID: user, PostKey: postKey, Contents: newContents}
	var reply tribrpc.ModifyTribbleReply
	err := ts.ModifyTribble(args, &reply)
	return err, reply.Status
}
...
// Modify tribble with invalid user
func testModifyTribbleInvalidUser() {
	pc.Reset()
	err, status := modifyTribble("invalidUser", "invalidPostKey", "invalidContent")
	if checkErrorStatus(err, status, tribrpc.NoSuchUser) {
		return
	}
	if checkLimits(10, 1000) {
...
// Modify invalid (non-existing) tribble
func testModifyTribbleInvalidPostKey() {
	userName := "user"
	createUser(userName)
	pc.Reset()
	err, status, _ := postTribble2(userName, "contents")
	if checkErrorStatus(err, status, tribrpc.OK) {
		return
	}
	err, status = modifyTribble(userName, "invalidPostKey", "invalidContent")
...
// Modify valid tribble
func testModifyTribbleValid() {
	userName := "tribUser50"
	createUser(userName)
	initTribbles := []tribrpc.Tribble{}
	numTribbles := 5
	postKeys := make([]string, numTribbles, numTribbles)
	for i := 0; i < numTribbles; i++ {
		initTribbles = append(initTribbles, tribrpc.Tribble{UserID: userName, Contents: fmt.Sprintf("contents%d", i)})
	}

	var err error
	var status tribrpc.Status
	for i := numTribbles - 1; i >= 0; i-- {
		err, status, postKeys[i] = postTribble2(initTribbles[i].UserID, initTribbles[i].Contents)
		if checkErrorStatus(err, status, tribrpc.OK) {
			return
		}
	}
	...
	newContent := fmt.Sprintf("new_contents")
	err, status = modifyTribble(userName, postKeys[numTribbles-1], newContent)
	if checkErrorStatus(err, status, tribrpc.OK) {
		return
	}
	// expect the same order, only that the last tribble has new content
	expectedTribbles := []tribrpc.Tribble{}
	for i := 0; i < numTribbles-1; i++ {
		expectedTribbles = append(expectedTribbles, tribrpc.Tribble{UserID: userName, Contents: fmt.Sprintf("contents%d", i)})
	}
	expectedTribbles = append(expectedTribbles, tribrpc.Tribble{UserID: userName, Contents: fmt.Sprintf("new_contents")})

	err, status, tribbles := getTribbles(userName)
	if checkErrorStatus(err, status, tribrpc.OK) {
		return
	}
	if checkTribbles(tribbles, expectedTribbles[:numTribbles]) {
		return
	}
	if checkLimits(15, 5000) {
		return
	}
	fmt.Println("PASS")
	passCount++
...
func testGetTribblesBySubscriptionFewTribbles() {
	...
	expectedTribbles := []tribrpc.Tribble{{UserID: "tribUser2", Contents: "contents"}, {UserID: "tribUser4", Contents: "contents"}}
	for i := len(expectedTribbles) - 1; i >= 0; i-- {
		postTribble(expectedTribbles[i].UserID, expectedTribbles[i].Contents)
	}
	pc.Reset()
...
func main() {
	tests := []testFunc{
		...
		{"testModifyTribbleInvalidUser", testModifyTribbleInvalidUser},
		{"testModifyTribbleInvalidPostKey", testModifyTribbleInvalidPostKey},
		{"testModifyTribbleValid", testModifyTribbleValid},
	...
	if err := initTribServer(flag.Arg(0), fmt.Sprintf("localhost:%d", *port), fmt.Sprintf("localhost:%d", *port+1)); err != nil {
