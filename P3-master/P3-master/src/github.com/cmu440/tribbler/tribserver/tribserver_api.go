...
type TribServer interface {
	...
	// ModifyTribble modifies the content of a tribble with the specified Postkey.
	// The TribServer should only replace the content while maintaining the timestamp of the original tribble.
	// Replies with status NoSuchUser if the specified UserID does not exist.
	// Replies with status NoSuchPost if the specified Postkey does not exist.
	ModifyTribble(args *tribrpc.ModifyTribbleArgs, reply *tribrpc.ModifyTribbleReply) error
